...
void ssd1306_rect(ssd1306_t *ssd, uint8_t top, uint8_t left, uint8_t width, uint8_t height, bool value, bool fill, uint8_t thickness)
{
    for (uint8_t t = 0; t < thickness; ++t)
    {
        ...
        {
            ssd1306_pixel(ssd, x, top + t, value);
            ssd1306_pixel(ssd, x, top + height - 1 - t, value);
        }
    }
    for (uint8_t t = 0; t < thickness; ++t)
    {
        ...
        {
            ssd1306_pixel(ssd, left + t, y, value);
            ssd1306_pixel(ssd, left + width - 1 - t, y, value);
        }
    }
...
void ssd1306_square(ssd1306_t *ssd, uint8_t top, uint8_t left, uint8_t width, uint8_t height, bool value, bool fill)
{
    for (uint8_t x = left; x < left + width; ++x)
    {
